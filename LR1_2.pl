...
sum([X],S) :- S is X.
sum([X|A], S) :- sum(A, S0), S is S0 + X.

forj1(S,N) :- member(B,N), findall(M,grade(B,_,M),X),sum(X,S). 
    
fori1(N) :- members(A,N),findall(K,student(A,K),X),write(A),nl,writeln(X),
    findall(M,forj1(M,X),L),sum(L,S),length(L,C), findall(R,subject(R,_),H),
    setof(D,members(D,H),G),length(G,V), E is S / (C * V), writeln(E).

g1 :- findall(A,student(A,_), L),setof(K,members(K,L),N),
	findall(_,fori1(N),_).

fori2(N) :- members(A,N),findall(K,grade(K,A,2),L),
	subject(A,F),writeln(F),writeln(L).

g2 :-  findall(A,subject(A,_), L),setof(K,members(K,L),N),
	findall(_,fori2(N),_).


forj3(M,N) :- member(B,N), findall(B,grade(B,_,2),X), 
	setof(A,member(A,X),J), length(J,M).

fori3(N) :- member(A,N),findall(K,student(A,K),L),
	writeln(A),findall(M,forj3(M,L),J), length(J,E),writeln(E).
    
g3 :- findall(A,student(A,_), L),setof(K,members(K,L),N),findall(_,fori3(N),_).
